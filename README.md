...
上图一张图同时表达**对外链路**（客户端 → 网关 → 后端）与**后端四层代码分层**：代码按"是否有业务语义"切四层——让控制层只编排不干活、能力层承载工具操作、资源层向外部依赖抽象。
...
1. **LLM 自主决策（chat 端点）**：第一轮带 3 个工具，由 LLM 判断问题该走哪条路——问标书正文 → `retrieve_knowledge`；问评分标准 → `get_dimension_rubric`；问工期 / 团队 / 资质等结构化数据 → `get_bid_structured_info`；问报价 → 不调工具，如实说明标书正文未含报价、需以商务应答/报价文件为准（报价评审走确定性公式，不在 AI 对话中作答，见 `review_service._calc_price_formula`）；闲聊 / 非文档问题 → 不调工具直接答（不浪费检索）。
...
`GET /api/contracts`（无鉴权）声明本系统的 LLM 评测接口与场景清单，供评测平台自动发现（错误事件回流与回归回推的闭环见[第五章](#五技术闪光点)第 3 条）：
...
### 1. 四层单向依赖的代码分层
整个后端按「交互层 → 控制层 → 能力层 → 资源层」四层单向依赖组织：交互层只做路由 / 鉴权 / 解析 / SSE 格式化（薄壳，不承载业务编排）；控制层**只编排不干活**——`chat_agent` 接管 AI 对话全部状态（加载历史 → 落库 → 组装上下文 → 编排 → 落库），业务服务编排专家匹配 / 围串标 / 评审流转；能力层承载工具与异步任务执行；资源层向 LLM / 向量库 / ORM 抽象。依赖边界被单测守护（`test_chat_agent.py` 固定门面语义）——新增功能放错层、交互层直连资源层，提交即被拦。各层模块归属、依赖规则与**当前收敛状态**详见[第二章](#二系统架构)。
...
### 3. 可观测接入：让每一次故障变成可复现的回归用例
系统接入统一的**可观测 SDK**，把运行期的失败转化为平台的评测资产，形成 `观测 → 聚类 → 组装 → 拉取 → 判定 → 回归回推 → 收口` 的闭环——**线上出过的错，会被自动固化成回归用例**。

**观测埋点**
- 每条业务链路落结构化事件：`trace_id` / `seq` / `interface` / `status` / `error_type` / `duration_ms` / `usage`（真实透出，不估算）；
- 本仓路由出口分散在 10 处，统一收敛为同一埋点入口，避免"某些分支悄悄没有观测"；
- 埋点写入**不阻断业务主链路**：观测属旁路，失败不改变业务响应。

**与评测平台的双向契约**
- **出**：本仓暴露标准契约清单（`agent` / `interfaces` / `scenes`），平台脚手架自动发现，**平台零特判**；
- **入**：错误事件回流至平台后自动聚类、生成回归用例，再由平台**回推**到本仓触发回归——形成"评测驱动修复"的闭环。

**闭环的可靠性设计**
- **断路器自愈**：熔断状态机原先在获取连接**之前**判定 `OPEN`，导致 `OPEN → HALF_OPEN` 永不迁移、只能重启容器恢复；修复为把半开判定抽成独立步骤，由获取路径与状态读取路径**共用**，自愈能力恢复；
- **先止血、再追根因**：可观测与降级策略保证依赖故障时**行为可预期**，而不是把失败伪装成成功。

**验证方式：主动故障注入**

不依赖线上真实事故，而是主动注入确定性故障（如把依赖域名指向黑洞地址制造 DNS / 连接类失败、在依赖侧制造超时），观察该 error 能否自动走完全部环节。该方式可重复、可对照，每一步均有落库产物可核。本仓**已用该方式完成端到端闭环验证**。

### 4. outbox 事务发件箱：MySQL → Neo4j 最终一致
...
### 12. 提示词外置：模板即程序输入，改文案不动代码
10 段提示词从 Python 字符串字面量外置为 `app/prompts/*.md`，由 `load_prompt` 统一读取。模板纳入 **Git 版本控制**：可 diff、可追溯、可按提交回滚（模板经 Dockerfile 打进镜像，回滚 = 切回旧提交后**重新构建并发布**，不是热回滚）。读取时机统一在**导入期**（`lru_cache`）——外置前是常量导入期读一次、builder 每次调用读一次，两种时机共存会制造「哪些模板改了文件立刻生效、哪些要重启进程」的歧义。加载器**不做插值**（带占位符的模板由调用方 `.format()` 填充），模板缺失直接 `FileNotFoundError`、**不兜底**：提示词少一段等于行为变了，静默降级比启动失败危险。三个被两个 builder 共用的安全 guard 仍留代码、模板以占位符引用——展开进模板等于同段防御文案存两份副本，改一份忘一份即注入防御静默失效。`.gitattributes` 为本目录锁死 LF：模板是程序读取、拼进 system 提示词的程序输入，它的**文件字节**值得钉住。但要说清**这不是修某个已知缺陷**——loader 走 `read_text(encoding="utf-8")`，文本模式默认 `newline=None` 会做换行归一化，CRLF 被吃掉，送到模型的文本不变。它钉住的是「字节即基准」这个不变量（谁把 `read_text` 换成 `read_bytes().decode()`，CRLF 才会漏过去且不报错），以及让按字节核对的验收结论跨平台可复现。

**怎么证明「外置」没偷偷改动任何字符：**
- **字节级往返等价**：用真实 builder 产出带哨兵串的提示词 → 抽出模板 → 再用 `.format()` 回填同一组哨兵，要求与原字面量**逐字节相等**（SHA-256 比对）。3 组评分 + 8 组对话 + 4 个常量全部相等——对送进模型的文本零改动。
- **比对函数产出，不是把字面量摆一起看**：把外置前那版文件按独立模块加载，与新版比对**函数产出**；肉眼看新旧字符串，漏掉一个空格也看不出来。
- **浏览器端真实触发**：单测绿 ≠ 模板真的送到了模型。`test_e2e_07_ai_scoring_path.py` 在真实栈 + 真实 DeepSeek 上，从专家工作台点「AI 辅助评分」跑通 LLM 评分支路（选技术维度，避开走公式出分的报价维度），取证为 AI 建议分 21.0/30.0、推理链含 `[RETRIEVING]→[REASONING]`、引用区为标书原文、无降级 Banner。它的价值在**补上原先的结构性空白**：01–06 号 E2E 的报价维度点「AI 辅助评分」走的是公式实时出分（等的是「报价公式」文案，不调 LLM）、其余维度直接人工填分，`score_system`（5 个占位符、含两个共享 guard）在浏览器端从未被触发过；`.md` 里若残留会炸 `.format()` 的字面大括号，端点只会把异常收成 error 帧，07 号正抓这个。
...
**后端代码分层**：整个后端按"是否有业务语义"切四层、依赖单向（下层绝不依赖上层）；上面目录树已逐行标注各目录所属层，分层表、依赖规则与当前收敛状态详见[第二章](#二系统架构)。

新增功能按职责归层：新增编排去 `agent/` 或 `services/`；新增工具去 `agent/tools.py`（注册进 `CHAT_TOOLS`）；新增任务去 `tasks/`；数据访问一律经 `models/` 或资源层服务。`test_chat_agent.py` 自动守护依赖方向。
...
- **契约优先**：对外接口先定契约、再实现（本项目架构约束）；LLM 相关接口如需评测平台发现，同步登记进 `api/contracts.py` 的清单。
